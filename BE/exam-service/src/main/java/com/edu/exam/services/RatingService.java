...
public class RatingService {
    ...
    public RatingResponse submitRating(String examId, String userId, int rating) {
        ...
        examRatingRepository.flush();
        ExamRatingRepository.RatingAggregateDto aggregate = examRatingRepository.getAggregateRating(examId);

        BigDecimal avgBd = aggregate.getAvg() == null
                ? BigDecimal.ZERO
                : BigDecimal.valueOf(aggregate.getAvg()).setScale(2, RoundingMode.HALF_UP);
        int count = aggregate.getCount();

        examRepository.updateRatingAggregate(examId, avgBd, count);

        return new RatingResponse(avgBd, count, rating);
    }

    public RatingResponse getRatingSummary(String examId, String userId) {
        ExamRepository.ExamRatingSummaryDto examSummary = examRepository.findRatingSummaryById(examId)
                .orElseThrow(() -> new ResourceNotFoundException("Exam", examId));

        BigDecimal avgBd = examSummary.getRating() == null ? BigDecimal.ZERO : examSummary.getRating();
        int count = examSummary.getRatingCount() == null ? 0 : examSummary.getRatingCount();
