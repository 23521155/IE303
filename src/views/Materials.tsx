    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-colors duration-300">
      ...
      <div className="flex space-x-4 mb-8 border-b border-gray-200 dark:border-slate-800">
        <button
          ...
          className={`pb-4 px-2 text-sm font-medium border-b-2 transition-colors ${
            activeTab === 'all' ? 'border-blue-600 text-blue-600 dark:text-blue-400 dark:border-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'
          }`}
        >
          {t('allMaterials')}
        </button>
        <button
          ...
          className={`pb-4 px-2 text-sm font-medium border-b-2 transition-colors flex items-center ${
            activeTab === 'pdf' ? 'border-blue-600 text-blue-600 dark:text-blue-400 dark:border-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'
          }`}
        >
        ...
        <button
          ...
          className={`pb-4 px-2 text-sm font-medium border-b-2 transition-colors flex items-center ${
            activeTab === 'video' ? 'border-blue-600 text-blue-600 dark:text-blue-400 dark:border-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'
          }`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredMaterials.map((material) => (
          <div key={material.id} className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col">
            <div className="relative h-48 overflow-hidden">
              ...
              <div className="absolute top-3 right-3 bg-white/90 dark:bg-black/80 backdrop-blur-sm px-2.5 py-1 rounded-md text-xs font-semibold text-gray-700 dark:text-gray-200 flex items-center shadow-sm">
                {material.type === 'pdf' ? <FileText className="w-3.5 h-3.5 mr-1.5 text-blue-600 dark:text-blue-400" /> : <Video className="w-3.5 h-3.5 mr-1.5 text-blue-600 dark:text-blue-400" />}
                {material.type.toUpperCase()}
              </div>
            </div>
            
            <div className="p-5 flex-1 flex flex-col">
              <span className="text-xs font-medium text-blue-600 dark:text-blue-400 mb-2 block">{material.category}</span>
