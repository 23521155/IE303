...
const INITIAL_POSTS = [
    ...
    },
    {
        ...
        content:
            'Có ai đang ôn thi JLPT N3 không ạ? Em bị mắc phần Đọc hiểu (Dokkai) quá, đọc mãi mà không bắt được Keyword. Mọi người có tips nào không chia sẻ cho em với 😭',
        image: null,
    ...
    return (
        <div className="min-h-screen bg-[#fafafa] dark:bg-[#121212] flex justify-center py-0 sm:py-6 px-0 sm:px-4 transition-colors duration-300">
            <div className="w-full max-w-[620px] bg-white dark:bg-[#1a1a1a] sm:rounded-3xl sm:border border-gray-200 dark:border-[#2d2d2d] sm:shadow-sm flex flex-col transition-colors duration-300 pb-10">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <div className="flex flex-col items-center">
                                        <Link href={`/profile/${post.author.id}`}>
                                            <img
                                                ...
                                                className="w-10 h-10 rounded-full object-cover shrink-0 border border-gray-100 dark:border-[#2d2d2d] hover:opacity-80 transition-opacity"
                                            />
                                        </Link>
                                    ...
                                    <div className="flex-1 min-w-0 pb-1">
                                        ...
                                        {post.image && (
                                            <Link
                                                href={`/blogs/${post.id}`}
                                                className="mt-3 block rounded-2xl overflow-hidden border border-gray-200 dark:border-[#2d2d2d]"
                                            >
                                                <ImageWithFallback
                                                    src={post.image}
                                                    alt="Post image"
                                                ...
                                                <Heart
                                                    className={`w-[22px] h-[22px] transition-colors ${
                                                        post.isLiked
                                                            ...
                                                            : 'text-gray-900 dark:text-gray-300 group-hover:text-gray-600 dark:group-hover:text-white'
                                                    }`}
                                                />
                                            </button>
                                            <Link
                                                href={`/blogs/${post.id}`}
                                                className="group p-2 rounded-full hover:bg-gray-100 dark:hover:bg-[#2d2d2d] transition-colors flex items-center justify-center"
                                            >
                ...
                {/* End of feed message */}
                {!hasMore && (
                    <div className="text-center py-8 text-gray-400 dark:text-gray-500 text-[15px]">{t.endOfFeed}</div>
                )}
            </div>
        </div>
    );
